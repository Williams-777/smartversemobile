...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  // NOTE: the backend has no update-profile endpoint yet (user-controller
  // only exposes GET /user/profile and DELETE /user/delete), so this can't
  // actually sync a name or phone number change to the server. It updates
  // the locally cached name so the rest of the app reflects it for this
  // session, and is honest with the user that it's local-only. Swap this
  // for a real AuthRepository.updateProfile() call once that endpoint exists.
  Future<void> _saveChanges() async {
    ...
    try {
      await getIt<AuthRepository>().updateLocalName(_nameController.text.trim());
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Saved on this device — syncing to your account isn't available yet")),
      );
            ...
            : SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 6.h),
                    Text(
                      "Editing isn't saved yet — there's no backend endpoint for this.",
